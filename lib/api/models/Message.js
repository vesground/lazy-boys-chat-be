...
const Message = (function() {
  ...
  async function getMessage({ id }) {
    ...
    const query = `messages:${id}*`;
    const messageData = await redisClient.hgetallAsync(query);
    const isMessageFound = !!messageData;

    if (!isDialogFound) throw Exception.create('messageNotExist', { serviceName: 'GET_MESSAGE', id });

    return messageData;
  };

  async function setIndexByDialogId({ id }) {
  ...
  return {
    ...
    getDialogByMembers: async function({ members }) {
      if (!members) throw Exception.create('missedParams', { serviceName: 'GET_MESSAGES_BY_MEMBER', missedParams:[ 'members' ]});
